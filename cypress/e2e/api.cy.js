...
describe('🔌 Testes de API', () => {
  ...
  describe('Produtos', () => {
    ...
    it('GET /api/products/search - Buscar produtos por nome', () => {
      cy.request('GET', `${baseUrl}/products/search?q=notebook`).then((response) => {
        expect(response.status).to.eq(200);
    ...
    it('GET /api/products/search - Buscar produtos por categoria', () => {
      cy.request('GET', `${baseUrl}/products/search?q=informática`).then((response) => {
        expect(response.status).to.eq(200);
    ...
    it('GET /api/products/search - Buscar produto inexistente', () => {
      cy.request('GET', `${baseUrl}/products/search?q=produtoinexistente`).then((response) => {
        expect(response.status).to.eq(200);
        expect(response.body).to.be.an('array');
  ...
  describe('Testes de Segurança da API', () => {
    ...
    it('Deve validar entrada para busca de produtos', () => {
      ...
      cy.request('GET', `${baseUrl}/products/search?q=<script>alert('xss')</script>`).then((response) => {
        expect(response.status).to.eq(200);
        expect(response.body).to.be.an('array');
      });
    });
